...
# This script creates "pure demand + training shaving" ReEDS load cases. The
# unshaved computing demand is constructed from installed capacity, spatial
# allocation, utilization, and PUE in the same way as the pure-demand script.
# A system-level daily shaving schedule is then selected for 2025, 2030, and
# 2035 using three maximum training-reduction fractions (5%, 10%, and 20%).
#
# For each target year and weather year, the optimizer sees:
#   net system demand before shaving
#       = baseline ReEDS load
#       + unshaved computing load
#       - baseline bulk renewable generation.
#
# The maximum removable load in each hour is the specified reduction fraction
# times the training-utilization component of computing demand. At most four
# hours in each 24-hour day may receive nonzero shaving.
#
# The daily objective is lexicographic, in this order:
#   (1) minimize the maximum absolute one-hour ramp;
#   (2) minimize the sum of absolute one-hour ramps;
#   (3) minimize the number of shaved hours;
#   (4) minimize total shaved load when all preceding criteria tie.
#
# Shaved load is removed from the modeled profile and is not reassigned to a
# different hour by this script. The resulting cases therefore represent
# shaving sensitivity cases, not an energy-conserving inter-hour shifting model.
#
# Array convention:
#   8,760 hours per weather year; seven weather years (2007-2013) are
#   concatenated to 61,320 hourly observations.
...
# Shaving is explicitly evaluated at the three reporting/planning years below.
# TARGET_YEAR_TO_INDEX maps those calendar years to rows in the 12-year capacity
# trajectory used by the rest of the model.
...
# Each value is the maximum fraction of the training-utilization component that
# may be removed in an eligible hour; it is not a fraction of total campus load.
...
# Resolve the data paths and the projection to be processed. Unlike the
# non-shaving script, this workflow writes only one selected national capacity
# projection per execution because shaving optimization is comparatively
# expensive and is evaluated separately for each projection when needed.
...
# Input readers
# -----------------------------------------------------------------------------
# Read national Low/Mid/High installed-capacity trajectories and return an
# array with shape [3 projection cases, 12 modeled years]. Capacity remains in
# the workbook unit here and is converted from GW to MW in run().
...
# Read total-utilization or training-utilization sheets as
# [7 weather years, 8,760 hours]. The seven weather-year blocks are later
# flattened in chronological block order to 61,320 values.
...
# Pair each total-utilization trace with its training-only trace. Exact sheet
# names are preferred. Positional matching is retained as a compatibility
# fallback for workbooks whose total and training sheets have corresponding
# order but different labels; a warning is emitted whenever that fallback is
# used.
...
# Build BA-to-state and BA-to-region indices and calculate each BA's share of
# its state's mapped area. State-level installed capacity is distributed with
# this area ratio, so mapped BA shares within a state sum to one.
...
# Load hourly base and improved PUE by EIA/ReEDS region. PUE is later blended
# by modeled year and applied to installed computing capacity before utilization
# is used to form hourly facility demand.
...
# -----------------------------------------------------------------------------
# Read one region's baseline bulk-renewable generation for a target modeled
# year and all seven weather years. The workbook uses a two-level column header
# (modeled year, weather year), which is matched explicitly here.
...
# Sum baseline bulk renewable generation across all modeled regions. This
# system-wide series is used only to construct the net-demand signal optimized
# by the shaving routine; it is not written as a separate load adjustment.
...
# -----------------------------------------------------------------------------
# Select the magnitude and timing of shaving within a single 24-hour day.
# Feasible shaving in hour h is bounded by max_shave_day[h], and no more than
# max_hours hours may be nonzero. To keep the search tractable, candidate hours
# are drawn from the endpoints of the day's largest baseline ramps plus hours
# with at least 10% of the day's maximum shaving capability.
#
# Candidate magnitudes are evaluated on nine evenly spaced fractions between
# zero and the hourly cap. Python tuple comparison implements the lexicographic
# objective (max ramp, total ramp, used hours, total shave). After a greedy
# build, two local-search passes refine magnitudes and test hour swaps.
def optimize_daily_shave_amount(
    ...
    # A 24-hour load vector contains 23 within-day consecutive-hour ramps.
    # The optimizer intentionally does not include the previous day's final hour
    # or the next day's first hour in this daily objective.
...
# Apply the 24-hour optimizer independently to every day in the concatenated
# weather-year series. Because 61,320 is divisible by 24, weather-year blocks
# remain aligned with daily boundaries.
...
# Core calculations
# -----------------------------------------------------------------------------
# Aggregate the baseline ReEDS load from BA level to a national/system total
# for each target modeled year. The original BA-resolved file is preserved for
# final output; this aggregate is used only by the shaving optimization.
...
# Build the system facility load corresponding to a utilization value of 1.0
# at every hour. This quantity includes installed capacity, spatial allocation,
# and PUE but not the utilization trace. Multiplying it by total or training
# utilization therefore converts those dimensionless traces directly to MW.
...
# Reconstruct BA-level computing demand after the system-level shave schedule
# has been converted to a utilization reduction. The same adjusted utilization
# trace is applied consistently across BAs; location-specific differences still
# enter through installed capacity, area allocation, regional PUE, and year.
...
# Copy the untouched baseline ReEDS HDF5, then add adjusted BA computing demand
# only for the target modeled years. Other datasets and metadata in the source
# file remain unchanged by construction.
...
# Main scenario loop: pair total/training profiles, build the system demand
# signal, optimize each reduction fraction for 2025/2030/2035, reconstruct BA
# demand, and write one HDF5 output per profile x shaving fraction. The ratio
# workbook records the amount removed relative to annual training utilization
# and annual total utilization.
    ...
    for ai_scenario_idx, (total_name, training_name, total_profile, training_profile) in enumerate(profile_pairs, start=1):
        ...
        for reduction_label, reduction_fraction in REDUCTION_SCENARIOS.items():
            ...
            for modeled_year in TARGET_MODELED_YEARS:
                ...
                # Hourly MW cap for the intervention. Only the training component is
                # eligible, while the total utilization profile defines original demand.
                max_shave_total = system_unit_ai_load[year_idx, :] * reduction_fraction * training_util_flat
                # Renewable generation can reduce the optimization signal to zero but
                # cannot create negative system demand in this implementation.
                ...
                # Convert the selected system MW reduction back to dimensionless
                # utilization so the same profile adjustment can be reconstructed by BA.
                ...
                # These two ratios are calculated from the national/system utilization
                # series. Region rows repeat the same values for reporting compatibility.
