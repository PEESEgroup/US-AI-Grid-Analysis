...
# This script constructs the "pure demand" load cases used as ReEDS inputs.
# The calculation proceeds in five stages:
#   1. Read national installed-computing-capacity trajectories (three projection
#      cases) and convert the reported GW values to MW.
#   2. Allocate each national trajectory to states using AI_Spatial.txt, then
#      allocate each state's capacity to its constituent balancing areas (BAs)
#      in proportion to BA area within that state.
#   3. Convert installed computing capacity to hourly facility electricity
#      demand by multiplying by the selected hourly utilization profile and the
#      regional hourly PUE trajectory.
#   4. Repeat each 8,760-hour PUE profile across the seven weather years
#      (2007-2013), while the utilization workbook supplies a distinct 8,760-hour
#      profile for each weather year.
#   5. Copy the baseline ReEDS HDF5 file and add the resulting BA-level demand
#      to every modeled-year dataset, preserving the original HDF5 structure.
#
# Array convention used throughout:
#   - 8,760 values = one weather year.
#   - 61,320 values = seven concatenated weather years (7 x 8,760).
#   - BA demand is stored internally as [BA, modeled_year, weather_hour].
#
# The script adds computing demand only. It does not subtract behind-the-meter
# generation and does not modify the baseline load file in place.
...
# Weather years follow the ReEDS chronological-weather representation used by
# the load and renewable inputs. They are concatenated rather than averaged.
...
# Resolve command-line paths while retaining defaults that reproduce the
# repository workflow. Candidate-path fallback is used only for files whose
# historical locations differ across ReEDS setups; scientific inputs otherwise
# keep explicit filenames.
...
# Input readers
# -----------------------------------------------------------------------------
# Read the three national capacity trajectories. The workbook is expected to
# contain 12 modeled years and the columns Year, Low-Case, Mid-Case, High-Case.
# The returned capacity array has shape [3 projection cases, 12 modeled years].
...
# Read every utilization-profile sheet except README. Each sheet must contain
# 8,760 rows and columns for weather years 2007-2013. The result for each sheet
# has shape [7 weather years, 8,760 hours]. Sheet order is retained and later
# determines the AI1, AI2, ... output labels.
...
# Convert the BA mapping table into the indices needed for demand allocation.
# Column names are detected defensively because historical BA_area files use
# different headers. Within each state, the BA allocation factor is:
#     BA area / sum of mapped BA areas in that state.
# Thus the state total is conserved when it is distributed across mapped BAs.
...
# Extract one 8,760-hour PUE series per EIA/ReEDS region from both the base-PUE
# and improved-PUE files. These lookup arrays are combined later using the
# modeled-year-dependent improvement weight.
...
# Confirm that the baseline HDF5 contains every modeled-year dataset and that
# each dataset uses the expected [61,320 weather-hours, number of BAs] layout.
# This protects the later in-place addition from silent dimension mismatch.
...
# Core calculation
# -----------------------------------------------------------------------------
# Construct BA-level facility electricity demand for one utilization scenario.
# For each BA and modeled year:
#   installed capacity (MW)
#     = national projection x state spatial share x within-state BA area share
#   facility demand (MW)
#     = installed capacity x utilization x PUE.
def build_ai_demand_for_profile(
    ...
    # Improvement adoption grows from 5% in the first modeled-year index by 20%
    # per subsequent index. This blends two exogenous hourly PUE trajectories;
    # it does not extrapolate or rescale the hourly shape itself.
...
# Preserve the complete baseline ReEDS file by copying it first, then add the
# new BA demand to each modeled-year dataset. Internal demand is [BA, year,
# hour], whereas the HDF5 dataset is [hour, BA], so a transpose is required.
def write_load_file(
    base_load_file: Path,
    output_file: Path,
...
# Orchestrate all projection/profile combinations. Three national capacity
# projections are crossed with every utilization sheet in the workbook. Output
# names encode the one-based profile number and projection number.
def run(config: Config) -> None:
    ...
    # The capacity projection workbook is interpreted in GW; ReEDS hourly load
    # datasets and the calculations below use MW.
