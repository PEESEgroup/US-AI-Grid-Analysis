...
# This script constructs ReEDS load cases in which computing campuses combine
# gross facility electricity demand with behind-the-meter wind, solar, and/or
# nuclear generation.
#
# Calculation sequence:
#   1. Read installed-computing-capacity projections and allocate national
#      capacity to states and then to balancing areas (BAs).
#   2. Convert BA computing capacity to gross hourly facility demand using the
#      selected utilization profile and regional PUE.
#   3. Calculate hourly on-site generation. Wind and solar use BA-specific
#      capacity-factor series; nuclear uses a fixed 0.92 capacity factor.
#   4. Compute campus net grid demand as
#          max(gross facility demand - on-site generation, 0).
#      The zero floor implements a strictly behind-the-meter assumption:
#      surplus on-site generation is curtailed in the load accounting rather
#      than exported as negative grid demand.
#   5. Copy the baseline ReEDS HDF5 file and add campus net grid demand at the
#      BA level for every modeled year.
#
# The wind/solar/nuclear scenario fractions are capacity fractions relative to
# installed computing capacity, not fractions of annual electricity use.
# Facility overhead enters gross demand through PUE; it does not scale the
# nameplate capacity assigned to on-site generation.
#
# Seven weather years (2007-2013) are concatenated, giving 61,320 hourly values
# for every modeled year and BA.
...
# Nuclear output is represented as a constant fraction of installed on-site
# nuclear nameplate capacity in every modeled hour.
NUCLEAR_CAPACITY_FACTOR = 0.92

# Canonical scenario names are kept separate from worksheet labels so output
# numbering remains stable even if workbook display labels contain spaces.
...
# Projection indices are zero-based (0/1/2 -> Low/Mid/High capacity paths).
# The mapping intentionally defines the exact scenario subset executed for each
# projection instead of taking the Cartesian product in all cases.
...
# Technology values are nameplate-capacity shares relative to installed
# computing capacity. Mixed cases sum the listed technology capacities; no
# storage or grid export is represented here.
...
# Resolve repository input paths and output behavior. The capacity-factor files
# have several accepted historical filenames; resolve_first_existing() allows
# the same scientific workflow to run across those directory conventions.
...
# Read BA-level wind or solar capacity factors. The file must contain BA names
# in 'columns' and hourly values in either 'data' or 'cf'. Only the first
# 61,320 rows are used, matching seven concatenated weather years.
...
# Read Low/Mid/High national installed-computing-capacity trajectories. The
# returned array is [3 projection cases, 12 modeled years]; run() later converts
# GW to MW before spatial allocation.
...
# Load all utilization scenarios from the workbook. Each scenario consists of
# seven 8,760-hour weather-year columns. Named scenario mapping below, rather
# than workbook order, determines which profiles enter each campus run.
...
# Build BA/state/region indices and the within-state area allocation factor.
# National capacity is first assigned to states by AI_Spatial.txt and then to
# each mapped BA by BA area / total mapped BA area in that state.
...
# Read hourly base and improved PUE for every region represented in the BA
# mapping. The two series are blended by modeled year in build_ai_gross_demand().
...
# Validate the baseline ReEDS HDF5 before any outputs are created. Every
# modeled-year dataset must have one row per concatenated weather hour and one
# column per BA.
...
# Core calculations
# -----------------------------------------------------------------------------
# Compute gross campus electricity demand before on-site generation.
# For BA b, modeled year y, and hour h:
#   gross[b,y,h] = installed_capacity[b,y]
#                  x utilization[h]
#                  x PUE[region(b),y,h].
# The PUE blend weight is 0.05 * 1.2**year_index.
def build_ai_gross_demand(
    ...
    # Blend the base and improved hourly PUE trajectories using a modeled-year
    # adoption weight that starts at 5% and grows by 20% per year index.
...
# Convert gross campus demand to grid-facing campus demand.
# Wind/solar nameplate capacity equals the scenario share times installed
# computing capacity and is multiplied by the corresponding BA capacity factor.
# Nuclear uses the same capacity basis with a constant 0.92 capacity factor.
# Net demand is floored at zero, so excess on-site generation is not exported
# to the bulk system in these cases.
def build_ai_net_demand_after_onsite_generation(
    ...
    for ba_idx in ba_mapping.valid_ba_indices:
        ...
        # On-site generation is sized from computing nameplate capacity, before
        # applying utilization or PUE. This keeps supply sizing separate from
        # hourly gross facility electricity consumption.
        onsite_generation = (
        ...
        # Strict behind-the-meter accounting: any generation above contemporaneous
        # campus demand is curtailed rather than represented as negative load/export.
        net_ba_demand[ba_idx, :, :] = np.maximum(gross_ba_demand[ba_idx, :, :] - onsite_generation, 0.0)
    return net_ba_demand


# Create each scenario from a fresh copy of the baseline HDF5 and add BA campus
# net demand to the corresponding modeled-year datasets. This avoids cumulative
# additions across scenarios and leaves the source baseline unchanged.
def write_load_file(
    base_load_file: Path,
    output_file: Path,
...
# Optional file-management helper only. It renames outputs that used the older
# renewable1...renewable9 tags to descriptive technology labels; it does not
# alter HDF5 contents or participate in the demand calculation.
...
# Cross the configured capacity projections, selected utilization scenarios,
# and nine on-site generation designs. AI_NAME_TO_LABEL supplies stable AI1...
# AI9 labels tied to the global scenario order. PROJ_TO_AI_SCENARIOS explicitly
# controls which utilization scenarios are evaluated for each projection; its
# ordering and any repeated entries are preserved exactly as configured.
def run(config: Config) -> None:
    ...
    # Convert national capacity projections from GW to MW before state and BA
    # allocation so units match the ReEDS load datasets.
