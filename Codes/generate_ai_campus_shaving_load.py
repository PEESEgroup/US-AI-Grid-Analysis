...
renewables/nuclear and ramp-minimizing training-load shaving.

"""

# This script combines two sensitivities in one ReEDS load construction:
# behind-the-meter campus generation and daily training-load shaving.
#
# For the selected capacity projection and utilization sheet, the workflow is:
#   1. Allocate installed computing capacity from the national projection to
#      states and then to balancing areas (BAs).
#   2. Build gross facility demand from capacity x utilization x regional PUE.
#   3. Calculate campus on-site wind/solar/nuclear generation and apply a
#      zero-export boundary when forming campus net demand.
#   4. Form the system net-demand signal used for shaving:
#          baseline load
#          + campus net demand before shaving
#          - baseline bulk renewable generation.
#   5. Select up to four shaved hours per day subject to a 5%, 10%, or 20%
#      hourly cap on the training-utilization component.
#   6. Convert the selected system-level shaved MW back to an equivalent
#      utilization reduction, apply that modified utilization profile to all
#      BAs, recompute campus net demand, and write the ReEDS HDF5 cases.
#
# The shaving objective is lexicographic: maximum one-hour ramp first, total
# ramp second, number of shaved hours third, and total shaved load fourth.
# Shaved energy is not reallocated to a later hour in this script.
#
# Baseline bulk renewable generation and campus on-site generation are treated
# separately. The former enters the system net-demand signal used to choose
# shaving hours; the latter directly reduces campus grid demand before that
# system signal is formed.
...
# Shaving outputs are created only for these three reporting years. The
# accompanying index map locates each year within the 12-year capacity pathway.
TARGET_MODELED_YEARS: Sequence[int] = (2025, 2030, 2035)
TARGET_YEAR_TO_INDEX: Mapping[int, int] = {2025: 1, 2030: 6, 2035: 11}
# Nuclear is represented with constant hourly availability at 92% of the
# scenario's on-site nuclear nameplate capacity.
...
# The reduction fraction applies to the training-utilization component only.
# All three cases retain the same maximum of four shaved hours per day.
REDUCTION_SCENARIOS: Sequence[Mapping[str, float | str]] = (
    {"label": "Shave05", "reduction": 0.05},
    {"label": "Shave10", "reduction": 0.10},
    {"label": "Shave20", "reduction": 0.20},
)

# On-site technology shares are nameplate-capacity fractions relative to
# installed computing capacity. The campus boundary does not include export.
...
class Config:
    ...
    target_ai_sheet: str = "Baseline E"
    overwrite: bool = True
...
# Resolve inputs for one selected capacity projection and one selected
# utilization sheet. Defaults reproduce the primary shaving configuration while
# command-line arguments allow the same routine to be applied to alternatives.
def parse_args() -> Config:
    default_data_dir = "Data Path" # Replace your path here
    parser = argparse.ArgumentParser(
        description="Generate AI-campus ReEDS load files with ramp-minimizing training-load shaving."
    )
    ...
    parser.add_argument("--projection-number", type=positive_int, default=2)
    parser.add_argument("--target-ai-sheet", type=str, default="Baseline E")
    ...
    ratio_output_file = args.ratio_output_file or data_dir / "AI_Campus_Shaving_Ratios_NetDemand.xlsx"
    ...
    return Config(
        ...
        target_ai_sheet=args.target_ai_sheet,
...
# Load 61,320 hourly BA capacity factors for wind or solar. BA column names are
# decoded once and converted to a name->column lookup to avoid repeated searches
# inside the scenario loops.
...
# Read Low/Mid/High national capacity trajectories as
# [3 projection cases, 12 modeled years]. The selected projection is converted
# from GW to MW and spatially allocated later in run().
...
# Read exactly one named total- or training-utilization sheet. The required
# 2007-2013 columns are returned as [7 weather years, 8,760 hours], preserving
# the same weather-year ordering used by ReEDS load and capacity-factor inputs.
def load_profile_sheet(workbook_path: Path, sheet_name: str) -> np.ndarray:
    ensure_file(workbook_path, "Utilization workbook")
    xls = pd.ExcelFile(workbook_path)
    if sheet_name not in xls.sheet_names:
        raise ValueError(f"Sheet '{sheet_name}' not found in {workbook_path}. Available sheets: {xls.sheet_names}")
...
# Construct BA/state/region lookup arrays and within-state area shares. These
# shares distribute each state's installed capacity across its mapped BAs while
# conserving the state total.
...
# Load base and improved hourly PUE by region. Modeled-year blending is applied
# consistently in both the system-level unit-load calculation and the BA-level
# reconstruction after shaving.
...
# -----------------------------------------------------------------------------
# Bulk renewable workbook readers
# -----------------------------------------------------------------------------
def get_ai_campus_run_name(projection_number: int, renewable_label: str) -> str:
    return f"_Proj{projection_number}_{renewable_label}_AI1"


# Read bulk-system renewable generation for one region, modeled year, and all
# seven weather years from the baseline generation workbook. This is distinct
# from campus on-site generation derived from wind/solar capacity factors.
def load_region_bulk_renewable_profile(
    workbook_path: Path,
    region_name: str,
    target_year: int,
    weather_years: Sequence[int],
) -> np.ndarray:
    ensure_file(workbook_path, "Bulk renewable generation workbook")
    ...
    for weather_idx, weather_year in enumerate(weather_years):
        ...
        if matching_col is None:
            raise ValueError(
                f"Missing bulk renewable column ({target_year}, {weather_year}) in {workbook_path.name}, sheet {region_name}."
            )
        values = pd.to_numeric(df[matching_col], errors="coerce").to_numpy(dtype=np.float32)
        values = values[~np.isnan(values)]
        if len(values) < HOURS_PER_YEAR:
            raise ValueError(
                f"Bulk renewable series too short in {workbook_path.name}, sheet {region_name}, "
...
# Sum baseline bulk renewable generation across all modeled regions to form the
# system generation term used in the shaving objective.
def build_system_bulk_renewable_total_from_workbook(
    workbook_path: Path,
    ...
    for region in sorted({str(r) for r in region_names}):
        if region in {"-1", "Summary", "", "nan"}:
            continue
        total += load_region_bulk_renewable_profile(workbook_path, region, target_year, weather_years)
    return total
...
# -----------------------------------------------------------------------------
# Solve the daily shaving heuristic under an hourly cap and a four-hour maximum.
# Candidate hours include both sides of the ten largest pre-shave ramps and
# hours with material shaving capability (>=10% of that day's maximum cap).
# Nine discrete shave fractions are tested for each candidate.
#
# Objective values are Python tuples, so comparisons are lexicographic:
# maximum ramp -> sum of ramps -> number of used hours -> total shaved load.
# A greedy selection is followed by two local refinement passes that retune
# selected magnitudes and test replacing selected hours with unused candidates.
def optimize_daily_shave_amount(
    ...
    # Only the 23 ramps internal to each 24-hour day are included in the daily
    # objective; cross-day boundary ramps are not part of this heuristic.
...
# Split the 61,320-hour series into independent 24-hour problems and concatenate
# their solutions. No constraint transfers shaved energy between days or hours.
...
# Core calculations
# -----------------------------------------------------------------------------
# Sum baseline BA loads to a system-total series for 2025, 2030, and 2035.
# System aggregation is required only for identifying the shaving schedule.
...
# Compute the system facility load at unit utilization. Because this series
# already contains capacity, spatial allocation, and PUE, multiplication by a
# dimensionless utilization trace yields system MW directly.
...
# Aggregate campus on-site generation across all BAs for a target modeled year.
# Wind and solar use local BA capacity factors; missing BA columns contribute
# zero for that technology. Nuclear is represented by a constant 0.92 capacity
# factor. Scenario shares are applied to installed computing capacity.
def build_system_onsite_generation_for_year(
    states_demand: np.ndarray,
    modeled_year: int,
    renewable: Mapping[str, float | str],
    ba_mapping: BAMapping,
    ...
    total = np.zeros(N_TOTAL_HOURS, dtype=np.float32)
    wind_share = float(renewable["wind"])
    solar_share = float(renewable["solar"])
    nuclear_share = float(renewable["nuclear"])
    for ba_idx in ba_mapping.valid_ba_indices:
        ...
        total += (
            installed_ai_capacity * wind_share * wind_cf
            + installed_ai_capacity * solar_share * solar_cf
            + installed_ai_capacity * nuclear_share * nuclear_cf
        )
    return total.astype(np.float32)


# Rebuild BA-level campus grid demand after shaving. Gross demand uses the
# modified utilization profile and regional PUE. On-site generation is then
# subtracted BA by BA and the result is floored at zero, preserving the
# no-export behind-the-meter assumption in the final ReEDS input.
def build_ba_campus_net_demand_for_year(
    states_demand: np.ndarray,
    modified_utilization_flat: np.ndarray,
    modeled_year: int,
    renewable: Mapping[str, float | str],
    ba_mapping: BAMapping,
    pue_base: Mapping[str, np.ndarray],
    ...
    ba_demand = np.zeros((n_ba, N_TOTAL_HOURS), dtype=np.float32)
    ...
    for ba_idx in ba_mapping.valid_ba_indices:
        ...
        gross_ai = installed_ai_capacity * modified_utilization_flat * pue_hour_all_weather
        ...
        ba_demand[ba_idx, :] = np.maximum(gross_ai - onsite_generation, 0.0)
    return ba_demand


# Start every output from the same baseline HDF5, add the reconstructed BA
# campus net demand for 2025/2030/2035, and leave all other modeled-year
# datasets unchanged.
...
# Evaluate every on-site generation design crossed with every shaving fraction
# for the selected projection and utilization sheet. Shaving hours are chosen
# from a system net-demand signal that includes baseline load, campus net demand,
# and baseline bulk renewable generation; final loads are reconstructed at BA
# resolution before writing.
#
# The ratio summary is system-wide: the same annual shaved/total and
# shaved/training utilization ratios are repeated for each region row so that
# the workbook keeps the region-year reporting layout used elsewhere.
def run(config: Config) -> None:
    ...
    total_profile = load_profile_sheet(config.utilization_file, config.target_ai_sheet)
    training_profile = load_profile_sheet(config.training_utilization_file, config.target_ai_sheet)
    ...
    total_util_den = float(total_util_flat.sum())
    training_util_den = float(training_util_flat.sum())
    ...
    for renewable in RENEWABLE_SCENARIOS:
        ...
        LOGGER.info("Processing on-site scenario %s", renewable_label)

        onsite_generation_by_year = {
            modeled_year: build_system_onsite_generation_for_year(
                states_demand,
                modeled_year,
                renewable,
                ba_mapping,
        ...
        run_name = get_ai_campus_run_name(config.projection_number, renewable_label)
        bulk_workbook_path = config.baseline_renewable_generation_file
        ensure_file(bulk_workbook_path, "Baseline renewable generation workbook")
        bulk_generation_by_year = {
            modeled_year: build_system_bulk_renewable_total_from_workbook(
                bulk_workbook_path,
                report_regions,
                modeled_year,
                WEATHER_YEARS,
        ...
        for shave in REDUCTION_SCENARIOS:
            shave_label = str(shave["label"])
            reduction_fraction = float(shave["reduction"])
            combo_label = f"{renewable_label}_{shave_label}"
            LOGGER.info("Optimizing %s", combo_label)

            modified_util_by_year: Dict[int, np.ndarray] = {}
            ...
            for modeled_year in TARGET_MODELED_YEARS:
                ...
                # Unshaved gross facility load uses total utilization. The separate
                # training profile is used only to set the hourly shaving ceiling.
                ...
                # Apply the same no-export campus boundary before constructing the
                # system net-demand signal used by the shaving optimizer.
                ai_net_before_shave = np.maximum(original_ai_total - onsite_generation_by_year[modeled_year], 0.0)
                # Baseline bulk renewable generation affects when shaving is valuable,
                # but it is not treated as campus-owned generation in the final BA load.
                net_total = np.maximum(base_total_flat + ai_net_before_shave - bulk_generation_by_year[modeled_year], 0.0)

                shaved_total_flat = optimize_shave_amount_by_day(net_total, max_shave_total, max_hours=4)
                shaved_util_flat = np.zeros(N_TOTAL_HOURS, dtype=np.float32)
                nonzero = system_unit_ai_load[year_idx, :] > 1.0e-8
                # Translate system MW shaving into a dimensionless utilization change;
                # this common adjustment is then used to rebuild demand BA by BA.
                ...
                modified_util_by_year[modeled_year] = np.clip(total_util_flat - shaved_util_flat, 0.0, None)

                ratio_train = float(shaved_util_flat.sum()) / training_util_den if training_util_den > 0 else 0.0
                ratio_total = float(shaved_util_flat.sum()) / total_util_den if total_util_den > 0 else 0.0
                # Ratios are system-wide annual quantities. Repeating them by region
                # preserves the expected region-year table structure for downstream use.
                ...
                LOGGER.info("%s | %d: %d shaved hours selected", combo_label, modeled_year, shaved_hours)

            ba_demand_by_year = {
                modeled_year: build_ba_campus_net_demand_for_year(
                    states_demand,
                    modified_util_by_year[modeled_year],
                    modeled_year,
                    renewable,
                    ba_mapping,
                    pue_base,
            ...
            output_file = config.output_dir / (
                f"EER_IRAlow_AI1_{renewable_label}_{shave_label}_proj{config.projection_number}_load_hourly.h5"
            )
            write_shaved_load_file(config.base_load_file, output_file, ba_demand_by_year, config.overwrite)
            ratio_tables[combo_label[:31]] = pd.DataFrame(sorted(ratio_rows, key=lambda row: (row["Year"], row["Region"])))
